...
var SampleValidator = (function () {
    ...
    function checkAddress(textValue, messages){
        var pattern=/[A-Za-z0-9'-_,\.\s]+$/;
        if(checkNotEmpty(textValue)) {
            ...
            messages.push("Deliver Address can't be empty.");
        }
    }

    function checkPostcode(textValue, messages){
        var pattern=/^([0-9]){4}?$/;
        if(checkNotEmpty(textValue)) {
            if (pattern.test(textValue)) {
                return;
            } else {
                messages.push("Invalid ZIP");
            }
        }else{
            messages.push("ZIP can't be empty.");
        }
    }

    function checkEmail(textValue, messages){
        ...
        if(checkNotEmpty(textValue)) {
            ...
            messages.push("Email Address can't be empty.");
        }
    }
    function checkDeliveryDetailsNotEmpty(messages){
        if(!checkNotEmpty($("#deliveryName").val())){
            messages.push("Delivery Name can't be empty.");
        }
        if(!checkNotEmpty($("#deliveryCity").val())){
            messages.push("City Name can't be empty.");
        }
    }
